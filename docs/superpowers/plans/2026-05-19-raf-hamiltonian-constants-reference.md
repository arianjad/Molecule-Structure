...
- [ ] **Step 2:** Field-interaction caveat box, B&C Ch6-cited. g_lp/g_l codebase mapping cited `[molecule_parameters.py:LL]` — state which codebase key maps to which B&C g; if ambiguous, `⚠[OPEN]` + Task-9 gate (do not assume the mapping).
    - **[per Arian, 2026-05-19]** g_lp↔B&C g_l′ mapping **pre-resolved** (no `⚠[OPEN]`, no Task-9 consult needed): codebase `g_lp` is the **parity-dependent anisotropic g-factor** g_l′, evaluated in the Curl-analog **p/(2B) convention** of `H_even_A` `ZeemanParityZ` `[hamiltonian_builders.py: H_even_A V_B block]` and the validated RaF A0 entry `[molecule_parameters.py: molecules['RaF']['boson']['A0']]`. Empirically confirmed by Steimle, Frey, Le, DeMille, Rahmlow, Linton, *Phys. Rev. A* **84** 012508 (2011), Table V (eq. 9 analog): measured g_l′(¹³⁸BaF A²Π,v=0) = **−0.536(23)** matches the Curl-analog estimate −p/(2B) = −0.2575/(2·0.21954) = −0.587 in sign and magnitude. Codebase BaF A0 now stores this measured value (`Molecule-Structure@2c2c104`, comment cites PRA84 012508 Tbl V). Mapping is unambiguous → Task 8 Step 2 writes the cell with this citation; Task 9 carries this as a `[per Arian, 2026-05-19]` ruling, not an OPEN item.
